...
- `files` — downloadable files attached to the subscription's plan (`PlanFile[]`).
- `links` — external links attached to the plan (`PlanLink[]`).
- `license` — issued licenses associated with the subscription (`SubscriptionLicense[]`).

```typescript
const sub = license.subscription; // e.g. from a license validation response
sub?.files?.forEach((f) => console.log(f.name, f.downloadUrl));
sub?.links?.forEach((l) => console.log(l.name, l.url));
sub?.license?.forEach((lic) => console.log(lic.licenseKey, lic.activationLimit));
```
