...
<Update label="v1.6.4" description="July 29, 2026">
...
#### `GET /refunds/`'s `search` param no longer matches the payment-provider refund ID

`search` now matches against refund ID, order ID, customer name, email, and customer ID only — it no longer matches the payment provider's refund ID (e.g. Stripe's `re_...` ID).
...
Three endpoints are now documented:
...
#### Checkout session endpoints now accept the Server API Key

`GET /checkout/sessions/`, `GET /checkout/sessions/{session_id}/`, and `GET /checkout/sessions/{session_id}/events/` previously didn't accept the standard Server API Key `Authorization: Bearer` credential used across the rest of the API — only session-based auth worked. These endpoints now accept the Server API Key like every other endpoint.
...
Four endpoints are now documented:
...
#### More endpoints now accept the Server API Key

`GET /orders/`, `GET /orders/{id}/`, `GET /orders/{id}/events/`, and `GET /webhook/logs/` previously didn't accept the standard Server API Key `Authorization: Bearer` credential used across the rest of the API — only session-based auth worked. These endpoints now accept the Server API Key like every other endpoint.

### Removed

#### `GET /orders/subscription/`

No longer documented — it does not accept the Server API Key and is not part of the supported public API.
...
#### Orders and Transactions endpoints now accept the Server API Key

`GET /orders/`, `GET /orders/{id}/`, and `GET /transaction/` previously didn't accept the standard Server API Key `Authorization: Bearer` credential used across the rest of the API — only session-based auth worked. These endpoints now accept the Server API Key like every other endpoint.
