...
The invoice object sent by `invoice.created`, `invoice.paid`, and `invoice.payment_failed` now includes `payment_link`. This signed URL takes the customer directly to the invoice payment page, so integrations can surface payment recovery without constructing a customer portal token themselves.
