...
rss: true
---
<Update label="v1.5" description="June 26, 2026">
...
We continued improving app APIs. These APIs help developers build custom flows on top of Kelviq.

Examples:

```text
Create checkout from your app
Validate licenses
Report usage
...
<Update label="v1.5" description="June 19, 2026">
...
We published app APIs for deeper product integrations. These APIs make it easier to connect Kelviq with your own product flows, admin tools, checkout flows, and customer dashboards.

You can use app APIs to build flows like:

- Create checkout sessions from your app
- Validate licenses
- Report usage
- Create customer portal sessions
- Sync customer state
- Handle subscription changes
- Show billing state inside your app
...
<Update label="v1.5" description="June 12, 2026">
...
```js
const result = await kelviq.licenses.validate({
  licenseKey: "LIC-123"
});
...
The Node SDK now supports `trial_end` when updating subscriptions. This gives developers more control over trials. This helps teams extend a trial after sales approval or shorten a trial when a customer upgrades early.
...
```js
await kelviq.subscriptions.update("sub_123", {
...
```js
const session = await kelviq.portal.sessions.create({
 customerId: "cus_123"
});
```
...
<Update label="v1.5" description="June 5, 2026">
...
Claude Sonet request: 3 credits
...
<Update label="v1.5" description="May 29, 2026">
...
<Update label="v1.5" description="May 22, 2026">
