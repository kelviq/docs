...
* When **Limit discount to selected products** is enabled, every restriction row must include a product before the discount can be created.
* Recurring discounts are enabled by default, with controls for a fixed number of billing cycles or a discount that applies forever.
* Discount fields no longer move the text cursor while you type.
...
## Flexible 28-day subscriptions

Kelviq now supports subscriptions that renew every 28 days. This gives sellers a predictable four-week billing option alongside monthly, quarterly, and annual billing.

The new period is supported throughout the billing flow, including checkout, subscriptions, orders, emails, usage resets, and analytics. Monthly recurring revenue now normalizes 28-day subscriptions to a monthly equivalent instead of treating one 28-day invoice as one month of revenue.
