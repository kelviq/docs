...
* `chargePeriod` (`ChargePeriod`): Billing period such as `MONTHLY`, `YEARLY`, `WEEKLY`, `DAILY`, or `TWENTY_EIGHT_DAYS`.
...
try {
    const response = await client.subscriptions.update({
      ...
      chargePeriod: CHARGE_PERIOD_CHOICES.MONTHLY,
      trialEnd: "2025-12-31T23:59:59Z" // or "now" to end the trial immediately
    });
} catch (e) {
