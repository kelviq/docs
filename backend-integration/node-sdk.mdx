...
Entitlement caching is enabled by default. The SDK keeps a per-process in-memory cache with a 60-second freshness window. Reads follow this order: fresh memory cache, optional distributed cache, then the Entitlement API. A successful API response refreshes both cache levels.

If the API is unreachable or returns a server error, the SDK returns the last known cached entitlement response when one exists. If the customer or feature has never been cached, the original request error is thrown.

```typescript
const client = new Kelviq({
  accessToken: process.env.KELVIQ_SERVER_API_KEY,
  enableCache: true,   // default: true
  cacheTtlMs: 60_000,  // default: 60 seconds
});
```
...
```typescript
const remaining = await client.reporting.flush();
```
...
The SDK currently supports the following operations:
    ...
    6. [Charges](/backend-integration/node-sdk#charges)

    7. [License](/backend-integration/node-sdk#license)

    8. [Portal](/backend-integration/node-sdk#portal)

    9. [Refunds](/backend-integration/node-sdk#refunds)

    10. [Payment Methods](/backend-integration/node-sdk#payment-methods)

    11. [Transactions](/backend-integration/node-sdk#transactions)
...
* `chargePeriod` (`ChargePeriod`): By default, organizations can use `MONTHLY`, `THREE_MONTHS`, `SIX_MONTHS`, `YEARLY`, and `ONE_TIME`. To enable `DAILY`, `WEEKLY`, or `TWENTY_EIGHT_DAYS`, contact [hi@kelviq.com](mailto:hi@kelviq.com).
...
```typescript
import { CHARGE_PERIOD_CHOICES, PAYMENT_BEHAVIORS, PRORATION_BEHAVIORS } from '@kelviq/node-sdk';
...
console.log(preview.nextInvoice?.lineItems);
```
...
Omit both `amountUnits` and `amount` to refund the order's entire remaining refundable balance. The amount must be positive and cannot exceed the order's remaining refundable balance — the SDK throws `InvalidRequestError` if `orderId` is missing, and the API rejects invalid amounts.
...
const refund = await client.refunds.retrieve({
  refundId: "7c2f3a91-2d4e-4a8b-9b1c-6f0a2e5d9c11",
});
```
...
console.log(page.results[0].amountTotal, page.results[0].morFeeBreakdown);
```
...
const returnUrl = encodeURIComponent("https://your-app.com/billing/done");
const addPaymentMethodLink =
    `${session.customerPortalUrl}billing` +
    `?token=${session.token}` +
    `&intent=add_payment_method` +
    `&customerId=cust_789` +
    `&returnUrl=${returnUrl}`;

res.redirect(addPaymentMethodLink);
```
...
| `subscription.trial_will_end` | A trial is approaching its end |
...
| `payment_method.created` | A payment method was added |
| `payment_method.updated` | A payment method was updated |
| `payment_method.deleted` | A payment method was removed |
