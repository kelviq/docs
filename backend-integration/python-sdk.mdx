...
Entitlement caching is enabled by default for both synchronous and asynchronous clients. The SDK keeps a per-process in-memory cache with a 60-second freshness window. Reads use a fresh memory entry first, then an optional distributed cache, and then the Entitlement API.

If the API is unreachable or returns a server error, the SDK returns the last known cached entitlement response when one exists. If the customer or feature has never been cached, the original request error is raised.

```python
client = Kelviq.create_sync_client(
    access_token=ACCESS_TOKEN,
    enable_cache=True,  # default: True
    cache_ttl=60.0,     # default: 60 seconds
)
```

Set `enable_cache=False` to disable caching.

### Share the cache across workers

The in-memory cache and offline reporting queue belong to one process and disappear when it restarts. Use the optional Redis store when multiple workers, processes, or containers need one shared, durable cache and queue.

```shell
pip install "kelviq-sdk[redis]"
```
...
For an asynchronous client, use `AsyncRedisStore`. Use the same prefix for every worker that should share state.
...
```python
remaining = client.reporting.flush()
# Async: remaining = await async_client.reporting.flush()
```

`remaining` is the number of reports still queued. Without Redis, the queue exists only in the current process.

See [When the Entitlement API is unavailable](/product-catalog/entitlements#when-the-entitlement-api-is-unavailable) for access-control fallback guidance.
...
The SDK currently supports the following operations:
    ...
    6. [Charges](/backend-integration/python-sdk#charges)

    7. [License](/backend-integration/python-sdk#license)

    8. [Portal](/backend-integration/python-sdk#portal)

    9. [Refunds](/backend-integration/python-sdk#refunds)

    10. [Payment Methods](/backend-integration/python-sdk#payment-methods)

    11. [Transactions](/backend-integration/python-sdk#transactions)
...
* `chargePeriod` (str | ChargePeriod): Required. The billing cycle for the subscription. You can use the `ChargePeriod` enum (e.g., `ChargePeriod.MONTHLY`) or a raw string. Must be one of:
        ...
        * `"TWENTY_EIGHT_DAYS"` / `ChargePeriod.TWENTY_EIGHT_DAYS`
...
**Optional Parameters:**
    ...
    * `trialPeriod` (int): Number of trial days to apply. Must be greater than or equal to `1` and overrides the selected plan's configured trial period.
    ...
    * `discountsEnabled` (bool): Whether the discount/coupon code field is shown on the checkout page. Defaults to `True`.

    * `lockEmail` (bool): When `True`, the email field is pre-filled and locked so the customer cannot change it. Defaults to `False`.

    * `defaultBillingCountry` (str): ISO 3166-1 alpha-2 country code (e.g. `"US"`, `"GB"`) used to pre-fill the billing address country on the checkout page.

    * `metadata` (dict): Arbitrary key-value pairs to attach to the checkout session. The keys are preserved exactly as supplied and the object is returned unchanged on the `checkout.completed` webhook's `metadata` field. Example: `{"order_ref": "ABC-123", "source": "pricing_page"}`.
...
for subscription in page.results:
...
```python
page = await async_client.subscriptions.list(
    customerId="cust_789",
    page=1,
    page_size=20,
)
...
* `customerId` (str): The client-defined identifier of the customer whose subscriptions will be returned.

**Optional Parameters:**

* `page` (int): Page number to retrieve.
* `page_size` (int): Number of subscriptions to return per page.
* `modifiedOnAfter` (str): Only return subscriptions modified on or after this ISO 8601 timestamp.
* `modifiedOnBefore` (str): Only return subscriptions modified on or before this ISO 8601 timestamp.

**Returns:**

A `PaginatedSubscriptionResponse` containing `count`, `next`, `previous`, and a `results` list of `SubscriptionData` models.
...
print(subscription.id, subscription.status)
for file in subscription.files:
    print(file.name, file.downloadUrl)
for link in subscription.links:
    print(link.name, link.url)
for license in subscription.license:
    print(license.licenseKey, license.activationLimit)
```
...
* `subscriptionId` (str): The Kelviq UUID of the subscription.

**Returns:**

A `SubscriptionData` model. A missing subscription raises `NotFoundError`.
...
```python
from kelviq_sdk import ChargePeriod
...
* `planIdentifier` (str): Identifier of the plan to subscribe to.
* `chargePeriod` (str | ChargePeriod): Billing period such as `MONTHLY`, `YEARLY`, `WEEKLY`, `DAILY`, or `TWENTY_EIGHT_DAYS`.
* `customerId` (str): Client-defined identifier of the customer.

**Optional Parameters:**

* `successUrl` (str): URL to redirect to after successful subscription creation.
* `features` (List[Dict[str, Any]]): Feature identifiers and requested quantities.
* `ipAddress` (str): Customer IP address used for location-based pricing.

**Returns:**

A `CreateSubscriptionResponse` model containing the created subscription, including its plan files, links, and issued licenses when available.
...
try:
    response = await async_client.subscriptions.update(
        ...
        trialEnd="2027-12-31 23:59:59",  # or "now" to end the trial immediately
...
* `trialEnd` (str): Optional. Controls the trial period for the updated subscription. Accepts either:
    * The literal string `"now"` to end any active trial immediately.
    * An ISO 8601 datetime string (e.g. `"2027-12-31 23:59:59"`) to set a new trial end date. The datetime must be in the future. Naive datetimes are interpreted as UTC.
    * If omitted, the existing trial behavior on the subscription is preserved.
* `paymentBehavior` (`Literal["activate_on_payment"]`): Optional. Set to `"activate_on_payment"` to keep the current subscription active while payment is pending. The new plan and features are applied only after payment succeeds. If the pending update expires, the current subscription remains unchanged. Omit this field to use the default immediate-update behavior.
* `prorationBehavior` (`ProrationBehavior`): Controls how a mid-period change is billed. Use `ProrationBehavior.IMMEDIATE_CHARGE`, `ProrationBehavior.PRORATE_NEXT_INVOICE`, or `ProrationBehavior.NO_PRORATION`. When omitted, the organization's default is used.
...
print(preview.nextInvoice.lineItems if preview.nextInvoice else [])
```

**Asynchronous:**

```python
preview = await async_client.subscriptions.preview_update(
    subscriptionId="78058918-9746-4280-9b9b-1bd5115eec6e",
    planIdentifier="premium-plan",
    chargePeriod="YEARLY",
    prorationBehavior=ProrationBehavior.NO_PRORATION,
)
```

Required parameters are `subscriptionId`, `planIdentifier`, and `chargePeriod`. Optional parameters are `prorationBehavior`, `features`, `ipAddress`, and `trialEnd`.

**Returns:** A `SubscriptionUpdatePreviewResponse` Pydantic model with recurring and immediate amounts in major and minor units, plus an optional next invoice and its line items.
...
The synchronous and asynchronous `charges` clients immediately charge a customer's saved payment method for a one-time plan. Use sandbox mode when testing this operation.
...
**Returns:** A `CreateChargeResponse` Pydantic model containing the charge record, amount, currency, status, product, plan, features, and customer details.

---

## Refunds

The synchronous and asynchronous `refunds` clients let you list, create, and retrieve refunds for orders owned by your organization.
...
```python
page = await async_client.refunds.list(status="SUCCEEDED", page=1, page_size=10)
```
  ...
  "results": [
    {
      ...
      "failureReason": null,
      "status": "SUCCEEDED",
      "order": { "id": "ORD-20260715123000-A1B2C" }
    }
  ]
}
```
...
* `start_date` : Include refunds created on or after this date (`YYYY-MM-DD`).
* `end_date` : Include refunds created on or before this date (`YYYY-MM-DD`).
* `page` : Page number to return.
* `page_size` : Number of results per page. Defaults to 10, maximum 100.

**Returns:** A `RefundListResponse` Pydantic model with `count`, `next`, `previous`, and `results: List[RefundResponse]`.
...
Omit both `amountUnits` and `amount` to refund the order's entire remaining refundable balance. The amount must be positive and cannot exceed the order's remaining refundable balance — the SDK raises `InvalidRequestError` if `orderId` is missing, and the API rejects invalid amounts.

**Returns:** A `RefundResponse` Pydantic model with `id`, `reason`, `amountUnits`, `amount`, `internalNote`, `failureReason`, `status`, and the complete serialized `order`.
...
```python
refund = client.refunds.retrieve("7c2f3a91-2d4e-4a8b-9b1c-6f0a2e5d9c11")
```

**Asynchronous:**

```python
refund = await async_client.refunds.retrieve("7c2f3a91-2d4e-4a8b-9b1c-6f0a2e5d9c11")
```
...
**Returns:** A `RefundResponse` Pydantic model, as above.

---

## Payment Methods

The synchronous and asynchronous `payment_methods` clients list saved payment methods for your organization.
...
```python
page = client.payment_methods.list(customer_id="geo-jacob", page_size=10)
```

**Asynchronous:**

```python
page = await async_client.payment_methods.list(customer_email="geojacob@example.com")
```
  ...
  "results": [
    {
      ...
      "isDefault": true,
      "status": "succeeded",
      "createdOn": "2026-04-02T06:12:04.123456Z",
      "modifiedOn": "2026-04-02T06:12:04.123456Z"
    }
  ]
}
```
...
* `customer_id` : Filter to payment methods belonging to the customer with this client-provided `customerId`.
* `customer_email` : Filter to payment methods belonging to the customer with this email address.
* `page` : Page number to return.
* `page_size` : Number of results per page. Defaults to 10, maximum 100.

Only payment methods with status `"succeeded"` are included in results.

**Returns:** A `PaymentMethodListResponse` Pydantic model with `count`, `next`, `previous`, and `results: List[PaymentMethodResponse]`. Each `methodData` dict is payment-provider-specific; for `card` it includes `brand`, `last4`, `expMonth`, `expYear`, `funding`, and `country`.

---

## Transactions

The synchronous and asynchronous `transactions` clients list financial transactions for your organization, ordered newest first.
...
print(page.results[0].amountTotal, page.results[0].morFeeBreakdown)
```

**Asynchronous:**

```python
page = await async_client.transactions.list(status="success")
```
...
```json
{
  ...
  "results": [
    {
      "id": "5b274952-0e54-48f6-aec6-e8a3f202b12e",
      "order": { "id": "ORD-20260715123000-A1B2C" },
      "createdOn": "2026-07-28T10:30:00Z",
      "currency": "USD",
      "currencySymbol": "$",
      "amountTotal": 100,
      "morFee": 4,
      "morFeeBreakdown": [
        { "name": "fixed_fee", "percentage": null, "valueUnits": 40, "currency": "USD" },
        { "name": "base_fee", "percentage": "2.9", "valueUnits": 290, "currency": "USD" },
        { "name": "subscription_fee", "percentage": "0.7", "valueUnits": 70, "currency": "USD" }
      ],
      "collectedTax": 0,
      "netPayout": 96,
      "status": "success",
...
* `start_date` : Include transactions created on or after this date (`YYYY-MM-DD`).
* `end_date` : Include transactions created on or before this date (`YYYY-MM-DD`).
* `page` : Page number to return.
* `page_size` : Number of results per page. Defaults to 10, maximum 100.

**Returns:** A `TransactionListResponse` Pydantic model with `count`, `next`, `previous`, and `results: List[TransactionResponse]`. Each transaction includes an itemized `morFeeBreakdown` (Merchant of Record fee components) when applicable — empty when no such fee applies.
...
`create_session(...)`
This operation creates a new portal session for a customer. Combine the returned `customerPortalUrl` and `token` into a signed link (`{customerPortalUrl}?token={token}`) and redirect the customer there.
...
portal_link = f"{session.customerPortalUrl}?token={session.token}"
...
* `customerPortalUrl` (str): The customer's billing portal base URL. **This URL alone does not authenticate the session** — append the `token` as a query parameter (`{customerPortalUrl}?token={token}`) to produce the signed link you share with the customer. The token expires, so create a fresh session per visit.
...
|---|---|---|
| `intent` | Yes | Set to `add_payment_method` to auto-open the add payment method form. |
| `returnUrl` | No | URL-encoded `https` URL of your application. After the payment method is saved, the customer is redirected here with `?kelviq_setup=success` appended. `http` is only allowed for `localhost` during development. |
| `customerId` | No | Opens the form for this specific customer account. If omitted, the customer's first account is used. |
...
return_url = quote("https://your-app.com/billing/done", safe="")
add_payment_method_link = (
    f"{session.customerPortalUrl}billing"
    f"?token={session.token}"
    f"&intent=add_payment_method"
    f"&customerId=cust_789"
...
See [Webhook event types](/guides/webhooks#event-types) for the complete list of supported events and their descriptions.
...
```
### Reading the environment from `KELVIQ_ENV`

`environment_from_env()` turns the `KELVIQ_ENV` convention shared with the Kelviq CLI and MCP server into the `environment` argument: `"sandbox"` when the variable is unset or empty, `"production"` or `"sandbox"` when it is set to one of those, and a `ValueError` naming any other value.

```python
import os
from kelviq_sdk import Kelviq, environment_from_env

client = Kelviq.create_sync_client(
    access_token=os.environ["KELVIQ_SERVER_API_KEY"],
    environment=environment_from_env(),  # reads KELVIQ_ENV: sandbox when unset
)
```

`environment="production"` and `environment="prod"` mean the same thing.
