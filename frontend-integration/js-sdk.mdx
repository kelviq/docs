...
<Warning>
**Security Warning:** Never use your Server API Key in the browser. Use the **Client API Key** which is safe to expose in client-side code. You can find both keys in the [API keys](https://app.kelviq.com/settings/api-keys) section of your Kelviq dashboard.
</Warning>
...
<Note>At least one of `customerId` or `productId` must be provided.</Note>
