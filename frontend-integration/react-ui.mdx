...
|------|------|-------------|
| `accessToken` | `string` | Your kelviq access token |
| `productId` | `string` | The unique identifier of the product to display the offering for |
...
| `pricingOptions` | `PricingOptions` | Yes | Price-formatting options |
...
// Price-formatting options (passed via the `pricingOptions` prop)
interface PricingOptions {
  localization: boolean;        // use the offering's locale/currency formatting
  showDecimal: boolean;         // show decimal places
  minimumDecimalDigits: number; // minimum decimal places
  maximumDecimalDigits: number; // maximum decimal places
  showCurrencySymbol: boolean;  // show the currency symbol (e.g. $)
  showCurrencyCode: boolean;    // show the currency code (e.g. USD)
}
```
