...
Use a Client API Key for supported browser and distributed-app integrations. For example, the license activation, validation, and deactivation endpoints accept either a Client API Key or a Server API Key. Never expose a Server API Key in client-side code.
...
| `page_size` | integer | Number of results to return per page. The maximum is `100`. |

Pagination defaults depend on the resource:

| Endpoints | Default `page_size` | Maximum `page_size` |
| --- | ---: | ---: |
| Products, Plans, Plan Versions, and Features | `100` | `100` |
| All other paginated APIs | `10` | `100` |

You can request fewer results by setting `page_size`. Values above `100` are capped at `100`.

```bash
curl "https://sandboxapi.kelviq.com/api/v1/customers/?page=2&page_size=10" \
  --header "Authorization: Bearer $KELVIQ_SANDBOX_SERVER_API_KEY"
```
...
{
  "count": 250,
  "next": "https://sandboxapi.kelviq.com/api/v1/customers/?page=3&page_size=10",
  "previous": "https://sandboxapi.kelviq.com/api/v1/customers/?page=1&page_size=10",
  "results": []
}
```
...
Search and filter parameters vary by endpoint and can be combined with pagination when supported.

Use the `search` query parameter for free-text search and endpoint-specific query parameters for filtering.

```bash
# Search customers
curl "https://sandboxapi.kelviq.com/api/v1/customers/?search=acme&page=1&page_size=10" \
  --header "Authorization: Bearer $KELVIQ_SANDBOX_SERVER_API_KEY"
