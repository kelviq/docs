...
### Review pricing changes like code

Propose a pricing change by editing `kelviq.config.ts` in a pull request — the reviewer sees exactly what changes: the new limit, the removed entitlement, the price that swapped. Today, once the PR is approved, you apply it in the [dashboard](https://app.kelviq.com) and run `kelviq pull` to confirm the applied state matches what was reviewed. When `push` ships, that last manual step becomes `kelviq push` — same file, same review, no transcription.
...
| [Dashboard](https://app.kelviq.com) | Visual management, analytics, customer operations, and (until `push` ships) applying catalog changes. |
...
- **`kelviq push`** — the write side: deploy a `kelviq.config.ts` to your account. Designed safety-first: a `--dry-run` diff preview, draft-only writes by default (nothing a customer sees changes without an explicit publish), and hard gates around publishing and customer migration. This completes the GitOps loop — pricing changes go PR → review → merge → push, with the dashboard as a viewer rather than the editor.
- **`kelviq promote`** — environment promotion: move your sandbox catalog to production in one command. Same engine and safety rails as `push` — a `--dry-run` diff of exactly what production would change, draft-first writes, and an explicit `--publish` gate — so "test in sandbox, promote to prod" becomes a reviewed, previewable step instead of a manual re-entry.
