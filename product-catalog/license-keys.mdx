...
The Activate, Validate, and Deactivate License APIs accept either a **Client API Key** or a **Server API Key** as a Bearer token:

```bash
Authorization: Bearer YOUR_API_KEY
```

Choose the key based on where the request originates:

| Request origin | API key | Guidance |
| --- | --- | --- |
| Browser, desktop app, mobile app, CLI, or plugin | Client API Key | Safe to include in distributed client software. The customer still supplies the license key. |
| Your backend or another trusted server | Server API Key | Keep it secret and load it from your server environment. |

Both key types can call `POST /license/activate/`, `POST /license/validate/`, and `POST /license/deactivate/`. Use a key from the same environment as the license: a sandbox key with a sandbox license, or a production key with a production license.

<Warning>
  Never expose a Server API Key in frontend code or software distributed to customers. Use a Client API Key for those integrations.
</Warning>

### 3.0 Reading a customer's key from your server

A customer's keys come back with their purchase records, authenticated with your server API key. Orders, subscriptions and invoices all carry a `license` array, and each entry holds `licenseKey` alongside `activationUsage`, `activationLimit` and `expiresOn`.

Which record to read depends on how the plan is sold:

- **One-time purchases** produce an order, not a subscription. Read `GET /orders/`, where each order's `billingType` is `ONE_TIME`. Orders have no customer filter — pass the customer's id or email as `search` to narrow the list to them.
- **Recurring plans** produce a subscription. Read `GET /subscriptions/?customer_id=<id>`.

This matters for how much you ask of the customer. A signed-in web app already knows who the customer is, so it can read the key server-side and activate on their behalf — there is no need to make them find an email and paste a key back into a session you control. Ask for the key only where the software has no way to look it up itself: a desktop app, CLI, plugin, or anything running off your server, which cannot hold a server API key.
...
**Authentication**: Client API Key or Server API Key
...
For software the customer signs in to, skip steps 3 and 4: read the key from their subscription as in [3.0](#3-0-reading-a-customer-s-key-from-your-server) and call Activate for them.
