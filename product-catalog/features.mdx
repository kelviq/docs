...
kelviq supports three types of features to model any business logic.
...
A Metered feature is for anything you want to bill based on consumption. This is the foundation of usage-based billing. kelviq supports two ways to track usage:

* **Pre-aggregated usage:** Your application sends kelviq a total usage count periodically. Think of this as telling us the final score of a game.
* **Raw events:** Your application sends individual events as they happen, and kelviq aggregates them for you. This is like sending us the play-by-play so we can calculate the score.
...
#### Raw Usage
Select this option when you want kelviq to calculate usage totals based on an event stream. The creation process involves several key stages:
```mermaid
graph LR
...
5.  **Configure Aggregation:** Specify how kelviq should calculate a single value from the filtered events. All methods besides `Count` require you to select an event property to aggregate.
...
* **Feature unit:** This is the base unit that your system reports to kelviq (e.g., singular `call`, plural `calls`).
...
When this setup is enabled, your system reports usage to kelviq in the base unit (`calls`), but all displays within kelviq (like paywalls and customer portals) and data sent to billing systems will use the customer-facing unit (`k calls`).
