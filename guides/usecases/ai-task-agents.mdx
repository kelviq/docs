...
description: "A step-by-step guide to configuring a hybrid pricing model for AI agents using kelviq, combining subscription tiers with usage-based charges."
---
...
This hybrid pricing model provides a balanced approach for AI Agent services. Customers get predictable base costs and the ability to scale their usage, while your business aligns revenue with the value delivered (tasks performed and agent capacity). kelviq' features for fixed charges, tiered usage-based billing, and customizable entitlements make it straightforward to implement such a model.