...
<Info>
**57 tools** spanning connection checks, customers, checkout, subscriptions, discounts, webhooks, entitlements, licenses, usage, offerings, promotions, the customer portal, and full catalog management (products, plans, features) — plus a resource for every docs page and the full OpenAPI spec, and **3 guided prompts** for the most common workflows. Every tool carries [MCP safety annotations](https://modelcontextprotocol.io/docs/concepts/tools#tool-annotations), so clients can auto-approve read-only calls while confirming destructive ones.
</Info>
...
The server runs locally on your machine as a Node process started by your AI client. It speaks the MCP **stdio transport** — your client launches `npx -y @kelviq/mcp-server@latest`, connects over stdio, and discovers the tools and resources advertised by the server.
...
Every client uses the same underlying command — `npx -y @kelviq/mcp-server@latest` — with your API keys and `KELVIQ_ENV` passed as environment variables. What differs is the config file location and, in a few cases, the root JSON key. Pick the tab for your client.

<Info>
The server uses your **sandbox** environment unless `KELVIQ_ENV=production`. Use keys from the same environment — to work with live data, use production keys and set `KELVIQ_ENV=production`.
</Info>
...
claude mcp add kelviq \
  ...
  --env KELVIQ_ENV=sandbox \
  -- npx -y @kelviq/mcp-server@latest
```
...
Verify the connection with `/mcp` inside a Claude Code session — you should see `kelviq ✓ Connected, 57 tools`.
</Tab>
...
```json
{
  "mcpServers": {
    "kelviq": {
      "command": "npx",
      "args": ["-y", "@kelviq/mcp-server@latest"],
      "env": {
...
| `KELVIQ_ENV` | No | `sandbox` | `sandbox` or `production`. Set to `production` to work with live data — switches both API hosts automatically. Use a key from the same environment: keys only work against their own environment. |
...
Wherever a tool takes a product or feature reference (`plan_create`, `plan_entitlements_add`, `discount_create`'s `appliesTo`, `offering_get_product`), it accepts the identifier or the UUID; the server resolves an identifier before calling the API.

### Connection

| Tool | Description |
|------|-------------|
| `connection_check` | One authenticated read with the server key. Returns the environment, both API hosts and whether the key is `ok`, `missing`, `rejected` or errored, never the key itself. The first thing to call when something returns 401/403. |
...
| Tool | Description |
|------|-------------|
| `discount_list` | List discount codes. Optional `search` across name and code, paginated. |
| `discount_create` | Create a percentage or fixed-amount code, optionally limited to a product (identifier or UUID) and its plans with `appliesTo`. Merchant of Record organizations only. |
| `discount_retrieve` | Fetch one discount, including how often it's been redeemed and whether it's active. |
| `discount_update` | Enable or disable a code with `isActive`. Nothing else can change after creation. |
| `discount_archive` | Archive a code so it can no longer be used. |

### Webhooks

| Tool | Description |
|------|-------------|
| `webhook_endpoint_list` | List endpoints with their URLs, events and enabled state. |
| `webhook_endpoint_create` | Register a public URL for the events you choose. Localhost and private addresses are rejected. |
| `webhook_endpoint_retrieve` | Fetch one endpoint. |
| `webhook_endpoint_update` | Change an endpoint's URL, events or enabled state. |
| `webhook_endpoint_delete` | Delete an endpoint. Deliveries stop immediately. |
| `webhook_log_list` | Delivery attempts from the last 30 days, with status codes and retries. |

Signing secrets are hidden in webhook tool results. Pass `revealSecret: true` only when you need the value — your app reads the secret from its env file, so it rarely needs to appear in a conversation.
...
| `plan_create` | Create a plan under a product (by identifier or UUID). Plan starts in `DRAFT`. |
...
| `plan_entitlements_add` | Attach feature entitlements, each feature by identifier or UUID. |
...
| URI pattern | Content |
|-------------|---------|
| `kelviq://docs/{section}/{slug}` | Every page in the Kelviq docs |
| `kelviq://openapi.json` | The full OpenAPI 3.0 spec (every operation and schema) |
...
### Tool returns HTTP 401 or 403

Ask your assistant to call `connection_check`: it reports the environment the server is using and whether the key was accepted, without showing the key. Almost always, the key belongs to the other environment: a sandbox key only works against sandbox, and a production key only against production.

- The server uses sandbox unless `KELVIQ_ENV=production`. If your key is a production key, add `KELVIQ_ENV=production` to the server's environment and reconnect.
- The error message includes a hint naming the environment the server is using, and the startup log shows it too (`kelviq-mcp ready (env=…`). Make sure it matches your key.
- If neither shows an environment, your client is running an outdated copy of the server. Use `npx -y @kelviq/mcp-server@latest` in your client config.

### Tool returns `{"error": {"status": "Bad request", "message": "Invalid product identifier"}}`

Your API key works but the identifier doesn't exist on this account. Most common cause: the identifier exists in the other environment — sandbox and production have separate catalogs. Check `KELVIQ_ENV`.
