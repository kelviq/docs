...
Verify the connection with `/mcp` inside a Claude Code session — you should see `kelviq ✓ Connected, 56 tools`.
</Tab>
...
| `discount_create` | Create a percentage or fixed-amount code, optionally limited to products or plans with `appliesTo`. Merchant of Record organizations only. |
...
Almost always, the key belongs to the other environment: a sandbox key only works against sandbox, and a production key only against production.
