...
Subscription webhook objects now include `end_date` inside `data.object`. The field contains an ISO 8601 date when the subscription has a known end date and is `null` while no end date is set.
...
When a subscription changes plans, Kelviq charges any prorated difference immediately and sends `subscription.plan_changed`. After the generated invoice is paid, Kelviq sends the related order and invoice events.
