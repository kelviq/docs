...
| `subscription.trial_will_end` | A subscription's trial period is ending soon. |
...
<Accordion title="Example: subscription.trial_will_end payload">
```json
{
  "id": "7c6d5e4f-3b2a-4190-8e7d-6c5b4a3f2e1d",
  "type": "subscription.trial_will_end",
  "created_at": "2026-08-08T05:25:34Z",
  "data": {
    "object": {
      "id": "a22f1b2a-b922-4a28-9b0e-75e826988b4b",
      "object": "subscription",
      ...
      "end_date": null,
      "billing_period_start_time": "2026-08-01T05:25:33Z",
      "billing_period_end_time": "2026-08-31T05:25:33Z",
      "trial_start": "2026-08-01T05:25:33Z",
      "trial_end": "2026-08-11T05:25:33Z",
      "customer": {
        "id": "1bb69d0f-39f8-4db8-aace-e003d6e5f3ed",
        "name": "Geo Jacob",
        "email": "geo@example.com",
      ...
      "features": null,
      "organization": "2a3664c0-cbd2-4f1c-a98f-1b4f65e3256c",
      "created_on": "2026-08-01T05:25:34Z",
      "modified_on": "2026-08-01T05:25:34Z"
    }
  }
}
```

`data.object` uses the same subscription payload shape as `subscription.created`/`subscription.updated`; the trial's remaining time is in `trial_end`. Kelviq sends this once, a few days before `trial_end`, mirroring Stripe's own `customer.subscription.trial_will_end` timing — it's skipped if the subscription has already converted to `active` or has no trial days left by the time Stripe's notice arrives.
</Accordion>
...
```json
{
  ...
  "data": {
    "object": {
      ...
      "payment_link": "https://portal.kelviq.com/acme-inc/pay/aW5fMVJwb1pvU0JzdEN6eW9jUzdpUG1pdGgz",
      "order_id": null,
...
```json
{
  ...
  "data": {
    "object": {
      ...
      "currency": "USD",
      "paid_at": null,
      "due_date": "2026-08-01T10:14:22Z",
      "attempt_count": 1,
      "next_payment_attempt": "2026-08-04T10:14:22Z",
      "failure_details": {
        "code": "card_declined",
...
`attempt_count`, `next_payment_attempt`, and `payment_link` are also included on `invoice.created` and `invoice.paid` payloads. `failure_details` (with `code`, `message`, and `payment_method_type`) describes the most recent failed attempt and is `null` once the invoice moves past that attempt (e.g. a later `invoice.paid` for the same invoice). `payment_link` is `null` whenever the invoice isn't currently payable (e.g. already paid).
</Accordion>
...
<Note>
Use `express.raw()` (not `express.json()`) in Node.js so that `req.body` contains the unmodified request bytes. Parsing the body before verification will break the signature check. On a framework built on the Fetch API (Next.js route handlers, Hono, Remix, Cloudflare Workers), read the body with `await request.text()` and take each header with `request.headers.get(...)`, since `request.headers` is a `Headers` object rather than a plain object.
</Note>
