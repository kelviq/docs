...
KELVIQ_SERVER_API_KEY=server-xxxxxxxx
KELVIQ_ENV=sandbox

# Frontend (can be exposed to client)
NEXT_PUBLIC_KELVIQ_CLIENT_API_KEY=client-xxxxxxxx
NEXT_PUBLIC_KELVIQ_ENV=sandbox

# Your app URL (for redirects)
NEXT_PUBLIC_APP_URL=https://yourdomain.com
```

Build with sandbox keys — keys only work against their own environment. When you go live, switch both keys to production keys and set both `KELVIQ_ENV` values to `production`.

<Warning>
Never expose your server API key to the frontend or commit it to version control.
</Warning>

### Point API Routes at the Right Environment

Create `lib/kelviq.ts` so every API route calls the host that matches `KELVIQ_ENV`:

```typescript
export const KELVIQ_API_URL =
  process.env.KELVIQ_ENV === "production"
...
import { KELVIQ_API_URL } from "@/lib/kelviq";

export async function POST(request: NextRequest) {
  const { customerId, name, email } = await request.json();

  const response = await fetch(
    `${KELVIQ_API_URL}/customers/`,
    {
      method: "POST",
      headers: {
...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(
    `${KELVIQ_API_URL}/subscriptions/?customer_id=${customerId}`,
    {
      headers: {
        Authorization: `Bearer ${process.env.KELVIQ_SERVER_API_KEY}`,
      },
...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(
    `${KELVIQ_API_URL}/subscriptions/${subscriptionId}/update/`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
async function handlePlanChange(newPlanId: string) {
  ...
  if (current) {
    ...
    alert("Plan change started. The new plan becomes active after payment succeeds.");
  } else {
    ...
    <html lang="en">
      <body>
        <KelviqProvider
          ...
          accessToken={process.env.NEXT_PUBLIC_KELVIQ_CLIENT_API_KEY!}
          environment={process.env.NEXT_PUBLIC_KELVIQ_ENV === "production" ? "production" : "sandbox"}
        >
          {children}
        </KelviqProvider>
      </body>
