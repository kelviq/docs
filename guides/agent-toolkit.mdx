...
**Verify**: run `/mcp` — you should see `kelviq ✓ Connected, 56 tools`. Then ask something Kelviq-shaped ("set up a Pro plan for my product") and watch the relevant skill activate.
...
1. **Tools** (from the MCP server) do the work — 56 typed API calls, each carrying [safety annotations](https://modelcontextprotocol.io/docs/concepts/tools#tool-annotations) so clients can auto-approve reads and confirm destructive writes.
